...
class OperaAlterTemplate(object):
    ...
    def OperaDbName(self):
        ...
        # 根据字段参数来查询数据库数据
        sql_select = "select id,name from AgentConfigTemplate where id != '{TemplateID}' limit 1;".format(
            TemplateID=self.info['parameterize']['AgentInputTemplateID'])
        ...
        if len(data) == 0:
            ...
            id = data[0][0]
            # 更新数据数据
            update_sql = "update AgentConfigTemplate set name = '{name}' where id= '{id}';".format(name=name, id=id)
            print(update_sql)
            update(update_sql)
            return self.info
