...
class OperaAlterHost(object):
    ...
    def OperaDbIp(self):
        ...
        # 根据字段参数来查询数据库数据
        sql_select = "select id,ip from AgentHost where id != '{HostID}' limit 1;".format(HostID=self.info['parameterize']['HostID'])
    ...
    def OperaDbName(self):
        ...
        # 根据字段参数来查询数据库数据
        sql_select = "select id,name from AgentHost where id != '{HostID}' limit 1;".format(HostID=self.info['parameterize']['HostID'])
        data = select(sql_select)
        if len(data) == 0:
            self.info['flag'] = False
            self.info['skipMeg'] = '重复场景mock----数据无可用测试书记'
