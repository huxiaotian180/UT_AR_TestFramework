...
from interface.dbio.serviceManager.DBIOUpdateService import DBIOUpdateService_put
...
class Test_serviceManager(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_DBIOUpdateService_put(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, DBIOUpdateService_put, log)
