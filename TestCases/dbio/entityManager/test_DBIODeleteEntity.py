...
from interface.dbio.entityManager.DBIODeleteEntity import DBIODeleteEntity_delete
...
class Test_entityManager(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_DBIODeleteEntity_delete(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, DBIODeleteEntity_delete, log)
