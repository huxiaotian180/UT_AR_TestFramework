...
from interface.ml.MachineLearning.MachineLearningGetTaskOverviewData import MachineLearningGetTaskOverviewData_get
...
class Test_MachineLearning(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_MachineLearningGetTaskOverviewData_get(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, MachineLearningGetTaskOverviewData_get, log)
