...
from interface.dataManager.logWareHouse.GetLogLibraryMapInformation import GetLogLibraryMapInformation_GET
...
class Test_logWareHouse(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_GetLogLibraryMapInformation_GET(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, GetLogLibraryMapInformation_GET, log)
