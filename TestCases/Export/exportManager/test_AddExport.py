...
from interface.Export.exportManager.AddExport import AddExport_post
...
class Test_exportManager(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_AddExport_post(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, AddExport_post, log)
