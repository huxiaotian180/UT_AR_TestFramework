...
from interface.ar_manager.AgentAudit.ShowAgentAuditFileCount import ShowAgentAuditFileCount_post
...
class Test_AgentAudit(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_ShowAgentAuditFileCount_post(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, ShowAgentAuditFileCount_post, log)
