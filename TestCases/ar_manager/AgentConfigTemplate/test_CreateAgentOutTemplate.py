...
from interface.ar_manager.AgentConfigTemplate.CreateAgentOutTemplate import CreateAgentOutTemplate_post
...
class Test_AgentConfigTemplate(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_CreateAgentOutTemplate_post(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, CreateAgentOutTemplate_post, log)
