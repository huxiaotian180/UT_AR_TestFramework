...
from interface.ar_manager.AgentAuth.DeleteAgentAuthInfo import DeleteAgentAuthInfo_delete
...
class Test_AgentAuth(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_DeleteAgentAuthInfo_delete(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, DeleteAgentAuthInfo_delete, log)
