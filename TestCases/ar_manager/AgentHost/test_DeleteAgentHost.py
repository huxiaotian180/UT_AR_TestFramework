...
from interface.ar_manager.AgentHost.DeleteAgentHost import DeleteAgentHost_delete
...
class Test_AgentHost(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_DeleteAgentHost_delete(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, DeleteAgentHost_delete, log)
