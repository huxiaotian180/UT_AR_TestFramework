...
from interface.ar_manager.AgentJob.AddAgentJobTemplateToHost import AddAgentJobTemplateToHost_post
...
class Test_AgentJob(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_AddAgentJobTemplateToHost_post(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, AddAgentJobTemplateToHost_post, log)
