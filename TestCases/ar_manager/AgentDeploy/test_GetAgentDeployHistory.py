...
from interface.ar_manager.AgentDeploy.GetAgentDeployHistory import GetAgentDeployHistory_get
...
class Test_AgentDeploy(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_GetAgentDeployHistory_get(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, GetAgentDeployHistory_get, log)
