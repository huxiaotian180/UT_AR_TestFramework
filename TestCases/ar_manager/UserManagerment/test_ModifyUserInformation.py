...
from interface.ar_manager.UserManagerment.ModifyUserInformation import ModifyUserInformation_PUT
...
class Test_UserManagerment(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_ModifyUserInformation_PUT(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, ModifyUserInformation_PUT, log)
