...
from interface.storageManager.indicesWareHouse.SearchIndexByType import SearchIndexByType_GET
...
class Test_indicesWareHouse(object):
    @allure.story(case_info['model'])
    @pytest.mark.parametrize('key', case_info['key'])
    def test_SearchIndexByType_GET(self, key):
        parameter = case_info['date'][key]
        info = TestCaseDateManger.dataManger(case_info, parameter, SearchIndexByType_GET, log)
